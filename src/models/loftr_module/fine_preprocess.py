...
class FinePreprocess(nn.Cell):
    ...
    def construct(self, feat_f0, feat_f1, feat_c0, feat_c1, hw_c0, hw_f0, coarse_match_ids):
        ...
        feat_f0_unfold = ops.unfold(feat_f0, kernel_size=(self.W, self.W), stride=stride,
                                    padding=self.W // 2)  # TODO to reshape and concat
        ...
        feat_f1_unfold = feat_f1_unfold.reshape(n, c, self.W ** 2, s).transpose(0, 3, 2,
                                                                                1)  # bs, c, ww, s -> bs, s, ww, c
        ...
        if self.cat_c_feat:
            ...
            coarse = ops.repeat_elements(ops.expand_dims(feat_c_win, axis=2), rep=W ** 2,
                                         axis=2)  # (bs, 2*num_coarse_match, ww, c)
            feat_cf_win = self.merge_feat(
                ops.cat([fine, coarse], axis=-1))  # (bs, 2*num_coarse_match, ww, 2c) -> (bs, 2*num_coarse_match, ww, c)
            feat_f0_unfold, feat_f1_unfold = ops.split(feat_cf_win, num_coarse_match,
                                                       axis=1)  # (bs, num_coarse_match, ww, 2c)

...
def mask_pts_at_padded_regions(grid_pt, mask):
    ...
    new_grid_pt = ops.where(mask.bool(), grid_pt, ops.zeros_like(grid_pt))
    return new_grid_pt
    ...
    Example:
        ...
        <BLANKLINE>
                 [[0., 1.],
                  [1., 1.]]]])
    """
    xs = ops.linspace(0, width - 1, width)
    ys = ops.linspace(0, height - 1, height)
    if normalized_coordinates:
        xs = (xs / (width - 1) - 0.5) * 2
        ys = (ys / (height - 1) - 0.5) * 2
    # generate grid by stacking coordinates
    base_grid = ops.stack(ops.meshgrid(xs, ys, indexing="ij"), axis=-1)  # WxHx2
...
def spvs_coarse(data, config, data_cols):
    ...
    """
    # 1. misc
    N, _, H0, W0 = data[0].shape  #
    _, _, H1, W1 = data[2].shape
    scale = config['LOFTR']['RESOLUTION'][0]
    scale0 = scale * data[8][:, None]  # if 'scale0' in data_cols else scale
    scale1 = scale * data[9][:, None]  # if 'scale1' in data_cols else scale
    ...
    grid_pt0_i = mask_pts_at_padded_regions(grid_pt0_i, data[15])  # 15
    grid_pt1_i = mask_pts_at_padded_regions(grid_pt1_i, data[16])  # 16
    grid_pt0_i = ops.stop_gradient(grid_pt0_i)
    grid_pt1_i = ops.stop_gradient(grid_pt1_i)
    ...
    def out_bound_mask(pt, w, h):
        con1 = (pt[..., 0] < 0).astype(ms.int16)
        con2 = (pt[..., 0] >= w).astype(ms.int16)
        con3 = (pt[..., 1] < 0).astype(ms.int16)
        con4 = (pt[..., 1] >= h).astype(ms.int16)
        return (con1 | con2 | con3 | con4).bool()
    ...
    # new codes: change dynamic shape
    ...
    new_conf_matrix_gt = row_ids * ops.tensor_scatter_elements(conf_matrix_gt, indices, ops.ones_like(indices).astype(conf_matrix_gt.dtype), axis=2)
    ...
    data_cols.extend(['spv_w_pt0_i', 'spv_pt1_i'])
    data.extend([w_pt0_i, grid_pt1_i])
